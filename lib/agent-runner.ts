...
import { recordAgentRunEvent } from './agent-run-events';
import {
  type AgentMode,
...
import {
  projectAgentMemoryIds,
  userAgentMemoryIds
} from './agent-memory-ids';
import {
  elapsedSince,
  errorAgentRun,
  logAgentRun,
...
async function executeUserAgentRun(run: AgentRunRecord) {
  ...
  const { resourceId: memoryResource, threadId: memoryThreadId } = userAgentMemoryIds(run.userId);
...
async function runMastraStream({
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : 'Agent request failed before response';
...
function readUserAgentPayload(payload: Prisma.JsonValue): UserAgentRunPayload {
  ...
  return {
    ...
    personalMcpUrl:
      typeof payload.personalMcpUrl === 'string' ? payload.personalMcpUrl : defaultPersonalMcpUrl(),
