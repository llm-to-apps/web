...
import { ActionIcon, Button, Checkbox, Group, Menu, Modal, Stack, Text, ThemeIcon, Title } from '@mantine/core';
      ...
      >
          <Stack>
            ...
            <Checkbox
              ...
              onChange={(event) => updateDeleteConfirmation('code', event.currentTarget.checked)}
            />
            <Checkbox
              checked={deleteConfirmations.data}
              label={t.desktop.confirmData}
              onChange={(event) => updateDeleteConfirmation('data', event.currentTarget.checked)}
            />
          </Stack>

          <FormActions>
            <Button disabled={isDeleting} onClick={() => setIsDeleteOpen(false)} variant="subtle">
              {t.desktop.cancel}
            </Button>
            <Button
              leftSection={<Trash2 size={16} />}
